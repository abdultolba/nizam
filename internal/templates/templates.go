...
func GetBuiltinTemplates() map[string]Template {
	return map[string]Template{
		...
		"clickhouse": {
			...
			Tags:        []string{"database", "analytics", "olap", "clickhouse"},
			Service: config.Service{
				Image: "clickhouse/clickhouse-server:{{.VERSION}}",
				Ports: []string{"{{.HTTP_PORT}}:8123", "{{.NATIVE_PORT}}:9000", "{{.MYSQL_PORT}}:9004"},
				Environment: map[string]string{
				...
				Volume: "{{.VOLUME_NAME}}",
				HealthCheck: &config.HealthCheck{
					Test:     []string{"CMD", "clickhouse-client", "--host=localhost", "--port=9000", "--user={{.DB_USER}}", "--password={{.DB_PASSWORD}}", "--query=SELECT 1"},
					Interval: "30s",
					Timeout:  "10s",
					Retries:  3,
				},
			...
			Variables: []Variable{
				...
				{
					Name:        "HTTP_PORT",
					Description: "Host port for HTTP interface",
					Default:     "8123",
					Required:    false,
					Type:        "port",
					Validation:  "^[1-9][0-9]{0,4}$",
				...
				{
					Name:        "NATIVE_PORT",
					Description: "Host port for native TCP interface",
					Default:     "9000",
					Required:    false,
					Type:        "port",
					Validation:  "^[1-9][0-9]{0,4}$",
				...
				{
					Name:        "MYSQL_PORT",
					Description: "Host port for MySQL compatibility interface",
					Default:     "9004",
					Required:    false,
					Type:        "port",
					Validation:  "^[1-9][0-9]{0,4}$",
